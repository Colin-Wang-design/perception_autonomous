...
calib_data = parse_calibration('calib_cam_to_cam.txt')

# Extract necessary parameters
P_rect_00 = calib_data['P_rect_00'].reshape(3, 4)
P_rect_01 = calib_data['P_rect_01'].reshape(3, 4)
T_01 = calib_data['T_01']

# Calculate baseline
baseline = np.linalg.norm(T_01[:3])

# Triangulation function
def triangulate(left_point, right_point, P_left, P_right):
    """
    Triangulate the 3D position of a point given its 2D positions in the left and right images.
    
    :param left_point: (x, y) coordinates in the left image
    :param right_point: (x, y) coordinates in the right image
    :param P_left: Projection matrix of the left camera
    :param P_right: Projection matrix of the right camera
    :return: (x, y, z) coordinates in 3D space
    """
    A = np.array([
        left_point[0] * P_left[2] - P_left[0],
        left_point[1] * P_left[2] - P_left[1],
        right_point[0] * P_right[2] - P_right[0],
        right_point[1] * P_right[2] - P_right[1]
    ])
...
def main():
    ...
    sequence_path = '/Users/maxbrazhnyy/Library/CloudStorage/OneDrive-DanmarksTekniskeUniversitet/Faculdade/DTU/Autonoums Systems/S3_Fall24/34759 Perception for Autonomous Systems/Project/34759_final_project_rect/seq_02'
        ...
        for track_id in list(trackers.keys()):
            if track_id not in detected_ids:
                ...
                if lost_trackers[track_id]['frames_lost'] > max_lost_frames:
                    ...
                    # Assuming left_center and right_center are the 2D positions in the left and right images
                    left_center = predicted_pos  # Replace with actual left image position
                    right_center = predicted_pos  # Replace with actual right image position
                    point_3d = triangulate(left_center, right_center, P_rect_00, P_rect_01)
